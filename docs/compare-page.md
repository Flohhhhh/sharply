...
- Server page at `src/app/[locale]/(pages)/(tools)/compare/page.tsx`:
...
- Pair-level counts (minimal, no rollups):
  ...
  - Trigger: `ComparePairTracker` client component increments a per-pair counter on page view when both items resolve.
    - Renders in `src/app/[locale]/(pages)/(tools)/compare/page.tsx` only when both `a` and `b` exist.
...
- ComparePage (server): `src/app/[locale]/(pages)/(tools)/compare/page.tsx`
