...
- `src/app/api/out/amazon/route.ts` re-parses the ASIN, builds the `sharply-20` affiliate destination via `~/lib/links/amazon.ts`, logs the click with `track("amazon_redirect")`, and returns a 307 redirect.
