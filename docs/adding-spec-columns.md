...
3. **Do NOT** generate migration files - maintainers will generate a consolidated migration when merging dev to main/staging.
4. Wire the editor UI:
...
6. Add the spec to the registry: update `src/lib/specs/registry.tsx` to include the new field with a **unique, stable field key** (separate from the inline English label), proper inline English label, formatting, and section grouping. This key is what the table and compare models use to match specifications across items and locales.
...
For paired numeric ranges, add each bound to the submit whitelist and numeric comparison keys, give the grouped registry field its own edit anchor, and reuse one formatter across the public spec table, contributor submission preview, and admin approval view. Keep search behavior explicitly scoped if the new range is display-only.
...
5. Display (`src/lib/specs/registry.tsx`): add the stable field key `cameraType` under Basic Information when `gearType === "CAMERA"`. In the registry code, the scoped key is `specRegistry.sections.core.fields.cameraType.label`; in locale files, the full JSON path is `gearDetail.specRegistry.sections.core.fields.cameraType.label`.
...
Sync your local database with the schema changes:

```bash
npm run db:push  # Syncs your local database with schema.ts changes
```

**Important**: Do not generate migration files. Maintainers will generate a consolidated migration when merging dev to main/staging. After migrations are merged, use `npm run db:migrate` to sync your local database.
