...
When list view is active, initial and incremental loading use `GearTableSkeleton` instead of card skeletons. It uses dark, rounded full-width row placeholders with generous vertical spacing rather than simulated cells, so its layout stays stable across table scopes and viewport widths; the rows pulse subtly and use a vertical bottom fade to keep long result loading states visually lightweight.
