...
- Provider avatars: Discord OAuth refreshes `users.discordImage` on Discord authentication. The return from a first-time Discord link also runs a server action that reads the newly linked account and synchronizes its current avatar before refreshing settings. Users can select Discord or Custom in profile settings; `users.image` remains the custom/legacy image. Google avatars remain legacy/custom images and are not synchronized.
- Unlink flow: “Disconnect” delegates to a server-side account-linking mutation. The provider’s account row is removed; the user record remains and other sign-in methods keep working. Disconnecting Discord always selects Custom while retaining the last Discord URL for a future reconnect.
