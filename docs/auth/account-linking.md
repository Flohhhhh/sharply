...
- Provider avatars: Better Auth stores the provider profile image in `users.image`. Avatar UI treats provider-hosted images the same way as custom uploaded profile pictures and renders them directly through the shared avatar component.
